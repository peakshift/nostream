...
export class SubscribeMessageHandler implements IMessageHandler, IAbortable {
  ...
  private async fetchAndSend(
    ...
    const abortableFintEvents = addAbortSignal(
  ...
  private canSubscribe(
    ...
    const maxSubscriptions =
      this.settings().limits?.client?.subscription?.maxSubscriptions ?? 0;
    if (
      maxSubscriptions > 0 &&
    ...
    const maxFilters =
      this.settings().limits?.client?.subscription?.maxFilters ?? 0;
    if (maxFilters > 0) {
